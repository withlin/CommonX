...
namespace ZooKeeperNet
{
    ...
    public class ZooKeeper : IDisposable, IZooKeeper
    {
        ...
        private readonly IBinarySerializer _binarySerializer;// = Scope.Resolve<IBinarySerializer>();
        ...
        {
            var d = _binarySerializer.Serialize(data);
            return Create(path, d, Ids.OPEN_ACL_UNSAFE, CreateMode.Persistent);
        }
        ...
        public void Delete(string path, int version=-1)
        {
            string clientPath = path;
        ...
        public T GetData<T>(string path, IWatcher watcher, Stat stat) where T:class
        {
            var data = GetData(path, watcher, stat);
        ...
        public T GetData<T>(string path, bool watch, Stat stat) where T:class 
        {
            var data = GetData(path, watch, stat);
            return _binarySerializer.Deserialize<T>(data);
        }
        /// <summary>
        ...
        public Stat SetData(string path, object data, int version = -1)
        {
            var d = _binarySerializer.Serialize(data);
            return SetData(path, d, version);
        }
        /// <summary>
