...
{
    public class ElementFilterTranslator : ExpressionVisitor
    {
        ...
        public ElementFilterTranslator(ICacheStrategy strategy)
        {
            _strategy = strategy;
        }
        ...
        protected override Expression VisitMember(MemberExpression m)
        {
            if (m.Expression != null && m.Expression.NodeType == ExpressionType.Parameter)
            {
                ...
                string prefix = _strategy.GetTablePrefixByField(propName);
                sb.AppendFormat("{0}.{1}", prefix, propName);
            ...
            if (m.Expression != null && m.Expression.NodeType == ExpressionType.Constant)
            {
                ...
                if (c != null)
                {
                    ...
                    else if (m.Type == typeof(IEnumerable<string>))
                    {
                        sb.Append("(");
